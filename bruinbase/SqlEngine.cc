...
RC SqlEngine::load(const string& table, const string& loadfile, bool index)
{
  ...
  if(!fileData.is_open()){
      //using the C I/O function
      fprintf(stderr, "We cannot open the load file %s\n", loadfile.c_str());
      return -1;
  }
  //create output file
  ...
  while(getline(fileData, line)){
    ...
    if ((rc = parseLoadLine(line, key, value)) != 0){
      fprintf(stderr, "We cannot do the parseLoadLine\n");
      break;
    }
    if((rc = rf.append(key, value, rid)) != 0){
      //Failed to write into a file
        fprintf(stderr, "We cannot do the write into the file\n");
